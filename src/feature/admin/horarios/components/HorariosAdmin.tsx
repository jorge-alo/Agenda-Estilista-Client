...
export const HorariosAdmin = ({ estilistas, estilistaId, setEstilistaId }: Props) => {
  ...
  const { data: horarios = [], isLoading } = useHorarios(estilistaId);
          ...
          {isLoading ? (
            ...
            <ul className="hor-admin-lista">
              {horarios.map((h: Horario) => (
                <li key={h.id} className={`hor-admin-item ${h.activo ? "" : "inactivo"}`}>
                  <div>
                    <div className="hor-admin-item-info">{dias[h.dia_semana]}</div>
